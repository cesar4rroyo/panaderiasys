...
<?php
	...
	foreach($dataCampos as $value){
?>
<tr class="<?php if($c%2==0) echo 'par'; else echo 'impar';?>"><th><?php echo $enzabezado[$x];?></th><td colspan="<?php echo $nro_cam;?>">
<?php echo umill($dato[strtolower($value['descripcion'])])?>
</td></tr>
<?php 
$x++;
}
	...
	foreach($datoOperaciones as $operacion){
		if($operacion["tipo"]=="C"){
			...
			if(count($ocultaO)>1){
				...
				if($vbool==1){
				?>
				<td align="center"><a href="#" title="<?php echo umill($operacion["comentario"]);?>" onClick="javascript:<?php echo umill($operacion["accion"]);?>(<?php echo $dato[1];?>);"><?php if($operacion["imagen"]!=""){echo "<img src='../img/".$operacion['imagen']."' height=16 border=0 />";}echo umill($operacion["descripcion"]);?></a></td>
               <?php }?>
            <?php
			}else{
		?>
<?php if($operacion["idoperacion"]!=$_GET['ocultaope']){?><td align="center"><a href="#" title="<?php echo umill($operacion["comentario"]);?>" onClick="javascript:<?php echo umill($operacion["accion"]);?>(<?php echo $dato[1];?>);"><?php if($operacion["imagen"]!=""){echo "<img src='../img/".$operacion['imagen']."' height=16 border=0 />";}echo umill($operacion["descripcion"]);?></a></td><?php }?>
		<?php
			}
		}
	}
