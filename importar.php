...
PHPExcel_Calculation::getInstance()->setCalculationCacheEnabled(False);
	...
	while($param==0) //mientras el parametro siga en 0 (iniciado antes) que quiere decir que no ha encontrado un NULL entonces siga metiendo datos
	{
		  ...
		  $datos[($i)] = array('plato'=> trim($objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue()),
                  ...
                  'precio'=>$objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue(),'idimpresora'=>1,
                  'precioventa2'=>$objPHPExcel->getActiveSheet()->getCell('D'.$i)->getCalculatedValue(),
                  'comentario'=>'',
                  'abreviatura'=>trim($objPHPExcel->getActiveSheet()->getCell('A'.$i)->getCalculatedValue()),'kardex'=>'N');
              }
		}
	...
	foreach($datos as $k => $v){
	   if(ob_get_length()) ob_clean();
        ...
        if($v["categoria"]!=$categoriaant){
            $rst=$objCategoria->insertarCategoria($_POST["cboSucursal"],trim(strtoupper($v["categoria"])), trim(strtoupper(substr($v["categoria"],0,3))), 0, 0, "", $_SESSION['R_IdSucursal'],1,1,'S');
            $dax  = $rst->fetchObject();
            $idcategoria = $dax->idcategoria;
            $categoriaant=$v["categoria"];
        }
        if(trim($v["unidad"])=="KG"){
        	$idunidad=3;
 	    ...
 	    $rst = $objProducto->insertarProducto($_POST["cboSucursal"], $objProducto->generaCodigo(), $v["plato"],$idcategoria, 0, $idunidad, 0, 2, '', 0, 0, 0, 0, 0, 0, 0, 0, $v["kardex"], "N", trim(($v["comentario"])), "", "N", "P",$v["abreviatura"],$v["idimpresora"]);
		$dax = $rst->fetchObject();
		...
		$res= $objListaUnidad->insertarListaUnidadSucursales($_SESSION['R_IdEmpresa'], $_POST["cboSucursal"], $dax->idproducto,$_SESSION['R_IdSucursal'], $idunidad, $idunidad, 1.00, $v["precio"], 0, $v["precio"], $v["precioventa2"], 'S');
		//INICIO BITACORA
