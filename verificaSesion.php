...
$cla=md5($_POST["txtClave"]);
$codigo=strtoupper($_POST["CAPTCHA_CODE"]);
$_SESSION['R_Inactividad']=60000000000000;
...
try {
	...
	$resultado = exec("net start postgresql-9.1");
	echo '<script>alert("Reiniciando...");history.go(-1);</script>';
	exit();
}
if(!($usu && $cla)){
...
if(!empty($datoSesion)){
    ...
    $_SESSION['R_Caja'] = "EPSON TM-U220 Receipt(2)";
	...
	while($datoparam=$rst->fetchObject()){
        ...
        //venta predeterminada tablet
        if($datoparam->idparametros==21){
            if($datoparam->valor=="S")
                $_SESSION["R_ModoTablet"]="PREDETERMINADO";
            else
                $_SESSION["R_ModoTablet"]="NORMAL";
        }
        //PRECIO VENTA
        if($datoparam->idparametros==22){
    		$_SESSION["R_PrecioVenta"]=$datoparam->valor;
        }
	}
     
    if($_POST["Origen"]=="Mozo"){
       header("Location: mainmozo.php");
       $_SESSION["R_Formulario"]="Mozo";
    }else{ 
    ...
    if($_POST["Origen"]=="Mozo"){
        echo "<script>alert('Cuenta no esta activa, consulte con el Administrador del Sistema');window.open('loginMozo.php','_self')</script>";
	    exit();    
    }else {
	   $_SESSION['R_ContSecure']=$_SESSION['R_ContSecure']+1;
