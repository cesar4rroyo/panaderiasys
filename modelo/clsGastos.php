...
class clsGastos extends clsMovimiento
{
	...
	function consultarapertura(){
		...
		WHERE TIPOMOVIMIENTO.descripcion = 'GASTOS'
		AND T.estado = 'N'
		...
		$apertura2=$this->consultarultimaapertura2();
		if($apertura2==0){
			$num_row=0;
		}else{
			$sql = $sql . " AND  T.idmovimiento >= ".$this->consultarultimaapertura2();
	...
	function consultarcierre($fecha)
	{
		...
		AND MOVIMIENTO.idcaja is null";
		//AND date( movimiento.fecha ) = '".$fecha."'
		if($this->consultarultimocierrefecha($fecha)==0){
			$sql = $sql . " AND date( movimiento.fecha ) = '".substr($fecha,0,10)."'";
		}else{
			$sql = $sql . " AND idmovimiento >= ".$this->consultarultimocierrefecha($fecha);
		}
	...
	function existenciamov(){
		$sql="SELECT * FROM (select * from MOVIMIENTO union select * from movimientohoy) T WHERE T.estado='N' and T.IdSucursal=".$_SESSION['R_IdSucursal']." AND T.IdTipoMovimiento=7";
		$rst=$this->obtenerDataSQL($sql);
		$num=$rst->rowCount();
		return $num;
	}
	...
	function montodeaperturasoles(){	
		...
		$sql="SELECT CASE WHEN sum(CASE WHEN tipo='I' THEN TOTAL ELSE -1*TOTAL END) IS NULL THEN 0 ELSE sum(CASE WHEN tipo='I' THEN TOTAL ELSE -1*TOTAL END) END as total
		FROM (select * from movimiento union select * from movimientohoy) as MOVIMIENTO
		INNER JOIN TIPOMOVIMIENTO ON MOVIMIENTO.idtipomovimiento = TIPOMOVIMIENTO.idtipomovimiento
		INNER JOIN CONCEPTOPAGO ON MOVIMIENTO.idconceptopago = CONCEPTOPAGO.idconceptopago
		WHERE TIPOMOVIMIENTO.descripcion = 'GASTOS'
...
function montodecierresoles($fecha,$idcaja=0,$idusuario=0){	
	...
	function consultarultimaapertura(){
	
		$sql="SELECT T.idmovimiento 
        FROM (select idmovimiento,numero,total,idtipomovimiento,idconceptopago,estado,idsucursal,fecha from movimiento 
        union select idmovimiento,numero,total,idtipomovimiento,idconceptopago,estado,idsucursal,fecha from movimientohoy) T
		INNER JOIN TIPOMOVIMIENTO ON T.idtipomovimiento = TIPOMOVIMIENTO.idtipomovimiento
		INNER JOIN CONCEPTOPAGO ON T.idconceptopago = CONCEPTOPAGO.idconceptopago
		WHERE TIPOMOVIMIENTO.descripcion = 'GASTOS'
		AND T.estado = 'N'
		...
		AND CONCEPTOPAGO.idConceptoPago = 1
		AND T.IDSUCURSAL = '".$_SESSION['R_IdSucursal']."' ORDER BY 1 DESC LIMIT 1";
	...
	function consultarultimaapertura2(){
		...
		AND CONCEPTOPAGO.idConceptoPago = 1
		AND T.IDSUCURSAL = '".$_SESSION['R_IdSucursal']."'";
		$sql.=" AND T.idmovimiento > ".$this->consultarultimocierre();
		$sql.=" ORDER BY 1 DESC LIMIT 1";
	...
	function consultarMovCaja($nro_reg, $nro_hoja, $order, $by, $id, $tipomovimiento, $numero, $fecha, $origencaja='CC',$idcaja=0,$idusuario=0,$idtipodocumento=0,$idconceptopago=0,$persona='',$comentario='',$cajero='')
 	{
       ...
       FROM (";
			...
			if($origencaja=='CC'){ 
				...
				$sql.=" AND T.idmovimiento >= ".$this->consultarultimaapertura();
	...
	function consultarGastosReporte($nro_reg, $nro_hoja, $order, $by, $id, $tipomovimiento, $numero, $fechainicio, $fechafin, $idusuario=0,$idtipodocumento=0,$idconceptopago=0,$persona='',$comentario='',$cajero='')
 	{
		if(parent::getTipoBD()==1){
			$descripcion = "%".$descripcion."%";
			$sql = "execute up_BuscarMovimiento ".$nro_reg.", $nro_hoja, '$order', $by, $id, '".$this->mill($descripcion)."'";
       ...
       idsucursalref, comentario, CASE WHEN situacion='O' THEN 'Ordenada' WHEN situacion='A' THEN 'Atendida' WHEN situacion='P' THEN 'Pagada' END as situacion, situacion as situacion2, m.estado, CASE WHEN m.idtipodocumento=13 THEN total ELSE 0 END AS ingreso, CASE WHEN m.idtipodocumento=14 THEN total ELSE 0 END AS egreso, CASE WHEN m.idtipodocumento=9 THEN CASE WHEN moneda='S' THEN total ELSE round(total*".$_SESSION['R_TipoCambio'].",2) END ELSE 0 END AS ingresos, CASE WHEN m.idtipodocumento=10 THEN CASE WHEN moneda='S' THEN total ELSE round(total*".$_SESSION['R_TipoCambio'].",2) END ELSE 0 END AS egresos, CASE WHEN m.tipopersona='P' THEN PM.nombres ELSE s.razonsocial END as persona,M.idcaja, C.numero as caja, SC.abreviatura as saloncaja, (PMU.apellidos || ' ' || PMU.nombres) as cajero,
       B.descripcion as banco,TI.descripcion as tipotarjeta,M.numerotarjeta,CASE WHEN M.idbanco IS NOT NULL AND M.idtipotarjeta=2 THEN M.TOTAL-TOTALPAGADO ELSE 0 END as montocredito,CASE WHEN M.idbanco IS NOT NULL AND M.idtipotarjeta = 1 THEN M.TOTAL-TOTALPAGADO ELSE 0 END as montodebito
  ...
  LEFT JOIN Tipotarjeta TI on TI.idtipotarjeta = M.idtipotarjeta WHERE 1=1 ";
    ...
    function consultarCierreSiguiente($idmovimiento,$idsucursal=0){
        if($idsucursal==0) $idsucursal=$this->gIdSucursal;
        $sql = "SELECT T.idmovimiento 
        from (select idmovimiento,idtipomovimiento,idconceptopago,estado,fecha,idsucursal,idcaja,idusuario FROM MOVIMIENTO UNION select idmovimiento,idtipomovimiento,idconceptopago,estado,fecha,idsucursal,idcaja,idusuario from movimientohoy) T
		INNER JOIN TIPOMOVIMIENTO ON T.idtipomovimiento = TIPOMOVIMIENTO.idtipomovimiento
    ...
    function consultarMovCajaLiquidacionGeneral($idpenultimocierre=0,$ultimocierre=0,$idsucursal=0){
        if($idsucursal==0) $idsucursal=$this->gIdSucursal;
...
//        $sql .= " and T.idmovimiento>75";
    ...
    function consultarLiquidacionEgresos(){
		...
		INNER JOIN TIPOMOVIMIENTO ON MOVIMIENTOHOY.idtipomovimiento = TIPOMOVIMIENTO.idtipomovimiento
		INNER JOIN CONCEPTOPAGO ON MOVIMIENTOHOY.idconceptopago = CONCEPTOPAGO.idconceptopago
		WHERE TIPOMOVIMIENTO.descripcion = 'GASTOS' and conceptopago.tipo='E'
		AND MOVIMIENTOHOY.estado = 'N'
		AND MOVIMIENTOHOY.IDSUCURSAL=".$_SESSION['R_IdSucursal']."
		AND MOVIMIENTOHOY.MONEDA='S'
		AND MOVIMIENTOHOY.idconceptopago<>2 AND (MOVIMIENTOHOY.idcaja>0 OR MOVIMIENTOHOY.idcaja is null)";
