...
class clsTipoCambio extends clsAccesoDatos
{
	...
	function insertarTipoCambio($idsucursal, $monto)
 	{ 	
		$sql = "execute up_AgregarTipoCambio '".$this->mill($idsucursal)."', '".$this->mill($monto)."'";
	...
	function actualizarTipoCambio($id, $idsucursal, $monto)
 	{
   		$sql = "execute up_ModificarTipoCambio $id, '".$this->mill($idsucursal)."', '".$this->mill($monto)."'";
		$res = $this->ejecutarSP($sql);
		if($res==0){
			return "Guardado correctamente";
		}else{
	...
	function eliminarTipoCambio($id)
 	{
   		$sql = "execute up_EliminarTipoCambio $id";
		$res = $this->ejecutarSP($sql);
		if($res==0){
			return "Guardado correctamente";
		}else{
	...
	function consultarTipoCambio($nro_reg, $nro_hoja, $order, $by, $id, $descripcion)
 	{
		if(parent::getTipoBD()==1){
			$descripcion = "%".$descripcion."%";
			$sql = "execute up_BuscarTipoCambio ".$nro_reg.", $nro_hoja, '$order', $by, $id, '".$this->mill($descripcion)."'";
			...
			$sql = "SELECT IdTipoCambio, IdSucursal, Fecha, Monto FROM TipoCambio WHERE 1=1 ";
			/*$sql = $sql . " AND Estado LIKE 'N' ";*/
			if($id>0){ $sql = $sql . " AND IdTipoCambio = " . $id;}
