...
require('PDO.class.php');	// required for this test

// database connection parameters
$host_db = 'pgsql:host=localhost;dbname=sicawebcesar';	// host and database name
$user = 'cesar';					// database user
...
function PDO_example(&$PDO, $first_query) {
	...
	while($result = $smtp->fetch(PDO_FETCH_OBJ))
		$output .= "[{$result->idusuario}] &nbsp; [{$result->nombreusuario}]<br />";
	echo $output;
...
PDO_example($mysql_PDO, 'CREATE TABLE mytable(id INT(10), generic_string VARCHAR(100))');
