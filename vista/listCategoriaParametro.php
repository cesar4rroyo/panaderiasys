...
function buscar(){
	...
	setRun('vista/listGrilla','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=CategoriaParametro&id_clase=<?php echo $id_clase;?>&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id){
	setRun('vista/mantCategoriaParametro','&accion=ACTUALIZAR&clase=CategoriaParametro&id_clase=<?php echo $id_clase;?>&Id=' + id,'cargamant', 'cargamant', 'imgloading03');
}
...
<body>
<?php
$objFiltro = new clsCategoriaParametro($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstCategoriaParametro = $objFiltro->obtenerTabla();
if(is_string($rstCategoriaParametro)){
	echo "<td colspan=100>Error al Obtener datos de Perfil</td></tr><tr><td colspan=100>".$rstCategoriaParametro."</td>";
}else{
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantCategoriaParametro', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
