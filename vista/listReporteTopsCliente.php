...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ",0, 0, 0, '" + document.getElementById("txtBuscar_Apellido_Nombre").value + "','" + document.getElementById("txtBuscar_NroDoc").value + "','" + document.getElementById("cbosexo").value + "','" + document.getElementById("cbocompartido").value + "',3,'" + document.getElementById("txtFechaInicio").value + "','" + document.getElementById("txtFechaFin").value + "','topsCliente'," + document.getElementById('chkJornada').checked + "," + document.getElementById('cboIdCaja').value;
	setRun('vista/listGrillaSinOperacionTops','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Persona&id_clase=<?php echo $id_clase;?>&funcion=Reporte&filtro=' + vValor + '&imprimir=SI&tiporeporte=Tops&titulo=Cliente&datografico=idpersona', 'grilla', 'grilla', 'img03');
}
function ordenar(id){
