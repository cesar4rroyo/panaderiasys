...
//echo $id_clase;
try{
$objMantenimiento = new clsRol($id_clase,$_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
}catch(PDOException $e) {
    echo '<script>alert("Error :\n'.$e->getMessage().'");history.go(-1);</script>';
	exit();
}

$rstRol = $objMantenimiento->obtenerTabla();
if(is_string($rstRol)){
	echo "<td colspan=100>Sin Informacion</td></tr><tr><td colspan=100>".$rstRol."</td>";
}else{
	$datoRol = $rstRol->fetchObject();
}

$rst = $objMantenimiento->obtenercamposMostrar("F");
$dataRol = $rst->fetchAll();

if($_GET["accion"]=="ACTUALIZAR"){
	$rst = $objMantenimiento->consultarRol(1,1,'2',1,$_GET["Id"],"");
	if(is_string($rst)){
...
CargarCabeceraRuta([["ACTUALIZAR",'vista/mantRol','<?php echo $_SERVER["QUERY_STRING"];?>']],false);
<?php }else{?>
CargarCabeceraRuta([["NUEVO",'vista/mantRol','<?php echo $_SERVER["QUERY_STRING"];?>']],false);
<?php }?>
$("#tablaActual").hide();
$("#opciones").hide();
</script>
...
<body>
    <div class="container">
        <form id="frmRol" action="" method="POST">
