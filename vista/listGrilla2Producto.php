...
$filtros_vista = explode(",", $_GET["filtro"]);
$nombre_filtros_vista = explode(",", $_GET["nombrefiltro"]);
$array_filtros = array();
foreach ($filtros_vista as $i => $filtro_vista) {
    $array_filtros[$nombre_filtros_vista[$i]] = str_replace("'", "",str_replace("' ", "",str_replace(" '", "",$filtro_vista)));
}
$_SESSION["R_filtrosbusqueda"] = $array_filtros;
...
<?php
	...
	foreach($datoOperaciones as $operacion){
		if($operacion["tipo"]=="C"){
			...
			if($operacion["idoperacion"]!=2 and $operacion["idoperacion"]!=3 and $dato['idsucursal']!=$_SESSION['R_IdSucursal']){
			?>
			<!--td align="center"><a href="#" title="<?php echo umill($operacion["comentario"]);?>" onClick="javascript:<?php echo umill($operacion["accion"]);?>(<?php echo $dato[1];?>,<?php echo $dato[2];?>);"><?php if($operacion["imagen"]==""){echo umill($operacion["descripcion"]);}else{echo "<img src='img/".$operacion['imagen']."' height=16 border=0 />";}?></a></td-->
