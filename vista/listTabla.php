...
function buscar(){
	...
	setRun('vista/listGrilla','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Tabla&id_clase=<?php echo $id_clase;?>&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id){
	setRun('vista/mantTabla','&accion=ACTUALIZAR&clase=Tabla&id_clase=<?php echo $id_clase;?>&Id=' + id,'cargamant', 'cargamant', 'imgloading03');
}
...
function verCampos(id){
	vValor = id + ", 0, '%%'";
	setRun('vista/listCampo','&id_tabla='+id+'&nro_reg=<?php echo $nro_reg;?>&nro_hoja=1&clase=Campo&id_clase=20&filtro=' +vValor, 'frame','frame', 'imgloading01');
}
function verOperaciones(id){
	vValor = id + ", 0, '%%'";
	setRun('vista/listRelacionOperacion','&id_tabla='+id+'&nro_reg=<?php echo $nro_reg;?>&nro_hoja=1&clase=RelacionOperacion&id_clase=24&filtro=' +vValor, 'frame','frame', 'imgloading01');
}
buscar();
</script>
</head>
<body>
<?php
$objFiltro = new clsTabla($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantTabla', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
