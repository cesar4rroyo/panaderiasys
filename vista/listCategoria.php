...
function buscar(){
	...
	setRun('vista/listGrilla2Categoria','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Categoria&id_clase=<?php echo $id_clase;?>&imprimir=SI&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
...
function actualizar(id, idsucursal){
	//if(id!=1){
		setRun('vista/mantCategoria','&accion=ACTUALIZAR&clase=Categoria&id_clase=<?php echo $id_clase;?>&Id=' + id+'&id_cliente='+idsucursal,'cargamant', 'cargamant', 'imgloading03');
	//}else{
	//	alert('No está permitido editar está categoría!!!');
	//}
}
...
function eliminar(id, idsucursal){
	if(id!=1){
		...
		alert('No está permitido eliminar está categoría!!!');
	}
}

function detalle(idcategoria, idsucursal){
	//alert(idproducto);
...
<body>
<?php
$objFiltro = new clsCategoria($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstCategoria = $objFiltro->obtenerTabla();
if(is_string($rstCategoria)){
	echo "<td colspan=100>Error al Obtener datos del Perfil</td></tr><tr><td colspan=100>".$rstCategoria."</td>";
}else{
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantCategoria', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&id_cliente=<?php echo $id_cliente;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
