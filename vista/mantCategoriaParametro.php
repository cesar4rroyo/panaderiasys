...
//echo $id_clase;
try{
$objMantenimiento = new clsCategoriaParametro($id_clase,$_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
}catch(PDOException $e) {
    echo '<script>alert("Error :\n'.$e->getMessage().'");history.go(-1);</script>';
	exit();
}

$rstCategoriaParametro = $objMantenimiento->obtenerTabla();
if(is_string($rstCategoriaParametro)){
	echo "<td colspan=100>Sin Informacion</td></tr><tr><td colspan=100>".$rstCategoriaParametro."</td>";
}else{
	$datoCategoriaParametro = $rstCategoriaParametro->fetchObject();
}

$rst = $objMantenimiento->obtenercamposMostrar("F");
$dataCategoriaParametros = $rst->fetchAll();

if($_GET["accion"]=="ACTUALIZAR"){
	$rst = $objMantenimiento->consultarCategoriaParametro(1,1,'2',1,$_GET["Id"],"");
	if(is_string($rst)){
...
function setParametros(){
	...
	getFormData("frmMantCategoriaParametro");
	
}
function aceptar(){
	if(setValidar("frmMantCategoriaParametro")){
		g_ajaxGrabar.setURL("controlador/contCategoriaParametro.php?ajax=true");
...
<form id="frmMantCategoriaParametro" action="" method="POST">
...
<?php
require("fun.php");
reset($dataCategoriaParametros);
foreach($dataCategoriaParametros as $value){
?>
...
<table><tr>
	...
	<td><a href="#" onClick="javascript:setRun('vista/listCategoriaParametro','&id_clase=<?php echo $_GET['id_clase'];?>&id_tabla=<?php echo $_GET['id_tabla'];?>','frame', 'frame', 'img02')"><?php echo $datoCategoriaParametro->descripcion; ?></a></td><td>></td>
	<td><?php echo $datoCategoriaParametro->descripcionmant; ?></td>
</tr></table>
</div>
