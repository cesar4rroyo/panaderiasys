...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ", 0, <?php echo $id_cliente;?>, '" + document.getElementById("txtBuscar_NroMesa").value + "','" + document.getElementById("txtBuscar_NroPersonas").value + "','" + document.getElementById("txtBuscar_Salon").value + "'";

	setRun('vista/listGrilla2','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Mesa&id_clase=<?php echo $id_clase;?>&imprimir=SI&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id, idsucursal){
	setRun('vista/mantMesa','&accion=ACTUALIZAR&clase=Mesa&id_clase=<?php echo $id_clase;?>&Id=' + id+'&id_cliente='+idsucursal,'cargamant', 'cargamant', 'imgloading03');
}
...
<body>
<?php
$objFiltro = new clsMesa($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstMesa = $objFiltro->obtenerTabla();
if(is_string($rstMesa)){
	echo "<td colspan=100>Error al Obtener datos del Perfil</td></tr><tr><td colspan=100>".$rstMesa."</td>";
}else{
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantMesa', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&id_cliente=<?php echo $id_cliente;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
        ...
        <div class="col s12 FiltrosCajero" id="busqueda">
            <div class="col s12 m6 l3">
                <div class="input-field inline">
                    <input id="txtBuscar_NroMesa" type="text" name="txtBuscar_NroMesa">
                    <label for="txtBuscar_NroMesa">Numero de Mesa</label>
                </div>
            </div>
            <div class="col s12 m6 l3">
                <div class="input-field inline">
                    <input id="txtBuscar_NroPersonas" type="text" name="txtBuscar_NroPersonas">
                    <label for="txtBuscar_NroPersonas">Numero de Personas</label>
                </div>
            </div>
            <div class="col s12 m6 l3">
                <div class="input-field inline">
                    <input id="txtBuscar_Salon" type="text" name="txtBuscar_Salon">
                    <label for="txtBuscar_Salon">Sal&oacute;n</label>
                </div>
            </div>
