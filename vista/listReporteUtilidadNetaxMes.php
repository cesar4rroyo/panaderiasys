...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ", 0, 4, '" + document.getElementById("txtBuscar").value + "','','" + document.getElementById("txtFechaInicio").value + "','" + document.getElementById("txtFechaFin").value + "',0,'M'";
	//SI SE AGREGA MAS ARGUMENTOS, MODIFICAR EL FILTRO EN ReporteVentaResumen
	setRun('vista/listGrillaSinOperacion','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Movimiento&id_clase=64&funcion=ReporteUtilidadNetaxMesoxSemana&filtro=' + vValor + '&imprimir=SI&tiporeporte=DinamicoResumen&titulo=de Utilidad Neta&origen=VentaxMesoxSemana&grafico=SI&datografico=mes', 'grilla', 'grilla', 'img03');
}
