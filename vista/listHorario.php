...
function buscar(){
	...
	setRun('vista/listGrilla2','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Horario&id_clase=<?php echo $id_clase;?>&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id,idsucursal){
	setRun('vista/mantHorario','&accion=ACTUALIZAR&clase=Horario&id_clase=<?php echo $id_clase;?>&Id=' + id+'&idsucursal='+idsucursal,'cargamant', 'cargamant', 'imgloading03');
}
...
function verOperacionesHorario(id,idsucursal){
	setRun('vista/listRelacionOperacionHorario','&nro_reg=<?php echo $nro_reg;?>&nro_hoja=1&id_cliente='+idsucursal+'&clase=RelacionOperacionHorario&id_perfil=' + id+'&id_empresa=<?php echo $id_empresa;?>','frame', 'frame', 'imgloading03');
}
...
<body>
<?php
$objFiltro = new clsHorario($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstHorario = $objFiltro->obtenerTabla();
if(is_string($rstHorario)){
	echo "<td colspan=100>Error al Obtener datos de Horario</td></tr><tr><td colspan=100>".$rstHorario."</td>";
}else{
	$datoHorario = $rstHorario->fetchObject();
}
$rstOperaciones = $objFiltro->obtenerOperaciones();
if(is_string($rstOperaciones)){
	echo "<td colspan=100>Error al obener Operaciones sobre Horario</td></tr><tr><td colspan=100>".$rstOperaciones."</td>";
}else{
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantHorario', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&idsucursal=<?php echo $id_cliente;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
