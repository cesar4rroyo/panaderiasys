...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ", 0, 2, '" + document.getElementById("txtBuscar").value + "','','" + document.getElementById("txtFechaInicio").value + "','" + document.getElementById("txtFechaFin").value + "',0,0," + document.getElementById("cboIdTipoDocumento").value + ",'" + document.getElementById("txtPersona").value + "','" + document.getElementById("txtResponsable").value + "','" + document.getElementById("txtComentario").value + "'";
	//SI SE AGREGA MAS ARGUMENTOS, MODIFICAR EL FILTRO EN ReporteVentaResumen
	setRun('vista/listGrillaSinOperacion','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Movimiento&id_clase=<?php echo $id_clase;?>&filtro=' + vValor + '&imprimir=SI&tiporeporte=VentaAgrupado&titulo=de Documentos de Venta&fechainicio=' + document.getElementById("txtFechaInicio").value + '&fechafin=' + document.getElementById("txtFechaFin").value, 'grilla', 'grilla', 'img03');
}
