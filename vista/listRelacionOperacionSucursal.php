...
$clase = "RelacionOperacionSucursal";
...
function actualizar(id){
	setRun('vista/mantRelacionOperacionSucursal','&id_tabla=<?php echo $id_tabla;?>&accion=ACTUALIZAR&clase=<?php echo $clase;?>&id_clase=<?php echo $id_clase;?>&Id=' + id,'cargamant', 'cargamant', 'imgloading03');
}
...
<body>
<?php
$objFiltro = new clsRelacionOperacionSucursal($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<div id="operaciones">
<?php

$rstRelacionOperacionSucursal = $objFiltro->obtenerTabla();
if(is_string($rstRelacionOperacionSucursal)){
	echo "<td colspan=100>Sin Informacion</td></tr><tr><td colspan=100>".$rstRelacionOperacionSucursal."</td>";
}else{
	$datoRelacionOperacionSucursal = $rstRelacionOperacionSucursal->fetchObject();
}
...
if(is_string($rstOperaciones)){
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
		<input type="button" title="<?php echo umill($operacion['comentario']);?>" name = "cmdNuevo" value = "<?php echo umill($operacion['descripcion']);?>" onClick="javascript:setRun('vista/mantRelacionOperacionSucursal', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&id_tabla=<?php echo $id_tabla;?>', 'frame','frame', 'img04');"> 
		<?php
		}
	}
}
...
<table><tr>
	...
	?>
	<td><a href="#" onClick="javascript:setRun('vista/listRelacionTablaSucursal','&nro_reg=10&id_cliente=<?php echo $id_cliente;?>&id_empresa=<?php echo $id_empresa;?>&id_clase=42&clase=SecuenciaCodigo&filtro=0,<?php echo $id_cliente;?>,\'%%\'','frame', 'frame', 'img05')"><?php echo $datoSucursal->razonsocial; ?></a></td><td>></td>
    <?php
	}
	if($_GET["id_tabla"]>0){
