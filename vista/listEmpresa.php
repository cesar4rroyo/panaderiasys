...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ",<?php if($id_clase==57) echo $_SESSION['R_IdEmpresa']; else echo '0';?>, '" + document.getElementById("txtBuscar_RazonSocial").value + "','" + document.getElementById("txtBuscar_RUC").value + "','" + document.getElementById("txtBuscar_Email").value + "','" + document.getElementById("txtBuscar_Telefonos").value + "'";
	setRun('vista/listGrilla','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Empresa&id_clase=<?php echo $id_clase;?>&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id){
	setRun('vista/mantEmpresa','&accion=ACTUALIZAR&clase=Empresa&id_clase=<?php echo $id_clase;?>&Id=' + id,'cargamant', 'cargamant', 'imgloading03');
}
...
function verSucursal(id){
	vValor = id + ", 0, '%%'";
	setRun('vista/listSucursal','&id_empresa='+id+'&nro_reg=<?php echo $nro_reg;?>&nro_hoja=1&clase=Sucursal&id_clase=<?php if($id_clase==57) echo '58'; else echo '40';?>&filtro=' +vValor, 'frame','frame', 'imgloading01');
}
...
<body>
<?php
$objFiltro = new clsEmpresa($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantEmpresa', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
    ...
    <div class="row" style="padding: 10px;margin-bottom: 0px;">
        <div class="col s12 FiltrosCajero" id="busqueda">
            ...
            <div class="col s12 m6 l2">
                <div class="input-field inline">
                    <input id="txtBuscar_RUC" type="text" name="txtBuscar_RUC">
                    <label for="txtBuscar_RUC">RUC</label>
                </div>
            </div>
            <div class="col s12 m6 l3">
                <div class="input-field inline">
                    <input id="txtBuscar_Email" type="text" name="txtBuscar_Email">
