...
function buscar(){
    ...
    setRun('vista/listGrillaPagoAnticipado',formData, 'grilla', 'grilla', 'img03');
}
...
function entregar(vid){
    if(confirm('Desea realizar la operacion?')){
        g_ajaxGrabar.setURL("controlador/contPagoAnticipado.php?ajax=true");
    	g_ajaxGrabar.setRequestMethod("POST");
    	g_ajaxGrabar.setParameter("accion", "ENTREGAR");
    	g_ajaxGrabar.setParameter("txtId", vid);
    	g_ajaxGrabar.response = function(text){
    		buscar();
    		alert(text);			
    	};
...
<div class="Botones" id="opciones">
    <div class="row">
            <div class="col s12 center">
                <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" type="button" data-position="bottom" data-delay="50" data-tooltip="NUEVO" onclick="javascript:setRun('vista/mantPagoAnticipado', 'accion=NUEVO&amp;id_clase=53', 'cargamant','cargamant', 'img04');$('#'+$(this).attr('data-tooltip-id')).remove();$('#tablaActual').hide();"><i class="material-icons right">note_add</i>NUEVO</button>
            </div>
    </div>
</div>
