...
<div class="row">
    <div class="col s12">
      <ul class="tabs blue" style="overflow-x: hidden;">
        ...
        $c=0;
        while($dato=$rst->fetchObject() and $c<7){
            echo '<li class="tab col s6 m3 l3 Tab-activo" id="'.$dato->idsalon.'"><a href="#" onclick="javascript:genera_cboMesas(\''.$dato->idsalon.'\',\''.$dato->abreviatura.'\');">'.$dato->abreviatura.'</a></li>';
            $c++;
        };
        ?>
      </ul>
...
function genera_cboMesas(idsalon,abreviatura){
    ...
    g_ajaxPagina.response = function(text){
        ...
        CargarCabeceraRuta([["Mesas","vista/frmCajero","&id_clase=0"]],true);
    };
    g_ajaxPagina.request();
}
genera_cboMesas(1,'<?php if($_SESSION['R_IdSucursal']) echo "CLIENTE";else echo "SA01";?>');
