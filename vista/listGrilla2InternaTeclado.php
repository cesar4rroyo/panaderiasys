...
?>
<th class="center" onClick="javascript:ordenar<?php echo $nombre?>('<?php echo umill($value['descripcion']);?>');"><?php echo umill($value['comentario'])?></th>
<?php 
}
//<<Fin
...
while($dato = $rst->fetch()){
	...
	$c+=1;
?>
<tr id='<?php echo $dato[1];?>-<?php echo $dato[2];?>' class="hoverable">
<?php
	foreach($dataCampos as $value){
?>
...
<?php
    ...
    if($_GET["modo"]=="multiple"){
        echo "<td><input type='text' id='txtProducto".$dato["idproducto"]."' name='txtProducto".$dato["idproducto"]."' size='5' value='0' /> </td>";
    }else{
     
	foreach($datoOperaciones as $operacion){
		if($operacion["tipo"]=="C"){
                    if($operacion["idoperacion"]==1){
                            $operacion["imagen"] = "check";
...
<button <?php echo $operacion["idoperacion"];?> type="button" style="margin-left: 10px;" 
            ...
            onClick="javascript:<?php echo umill($operacion["accion"]);?>(<?php echo $dato[1];?>,<?php echo $dato[2];?>);"><i class="material-icons <?php echo $operacion["color"];?>-text text-darken-4"><?php echo $operacion["imagen"];?></i></button>
		<?php
		}
	}
    }
...
?>
<div class="col s12 m12 l2 right right-align"><?php if($nro_registros_total==0){echo "No hay registros&nbsp;<a href='#' class='btn-floating red tiny' onclick='javascript: ocultarResultadoListGrillaInterna()' title='Cerrar'><i class='material-icons'>clear</i></a>";}else{echo "Registros del $inicio al $fin (".$mostrar.") de ".$nro_registros_total;}?></div>
<div class="col s12 m12 l8 offset-l2 center">
        <ul class="pagination">
            <li class="disabled"><a href="#!"><i class='material-icons'>chevron_left</i></a></li>
<?php
//$ini = '<li class="waves-effect"><a href="#!"  onClick="buscarGrilla(';
$ini = '<li class="waves-effect"><a href="#!"  onClick="buscarGrillaInterna(';
$medio=');">';
if($nro_hojas>11){
	for($i=1;$i<=3;$i++){
...
function buscarGrillaInterna(nro_hoja){ 
	...
	eval("buscar<?php if($nombre=="Producto") echo $nombre."()";else echo $nombre."()";?>;");
}
</script>