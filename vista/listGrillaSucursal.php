...
eval("\$objGrillaHorario = new clsHorario(60, 1,\"".$_SESSION['R_NombreUsuarioCloud']."\",\"".$_SESSION['R_ClaveCloud']."\");");
...
while($dato = $rst->fetch()){
	...
	$c+=1;
?>
<tr class="<?php if($c%2==0) echo 'impar'; else echo '';?>"><td>
...
<?php
$rst2 = $objGrillaHorario->buscarHorario(NULL,$dato['idsucursal'],NULL);
if(is_string($rst2)){
...
<tr>
<td>Horario de Atenci&oacute;n:</td><td class="texto1" colspan="3">
<?php
while($dato2 = $rst2->fetch()){?>
	<?php
    if($dato2['horainicio']<>$horainicio or $dato2['horafin']<>$horafin){
        if($cont>0){
			if($dia<>$dia1){
				?>
				<?php echo umill(ucwords(strtolower(substr($dia1,2))));?>-<?php echo umill(ucwords(strtolower(substr($dia,2))));?>&nbsp;<?php echo umill(substr($horainicio,0,5));?>-<?php echo umill(substr($horafin,0,5));?><br>
				<?php
				$dia1=$dato2['dia'];
				$horainicio=$dato2['horainicio'];
				$horafin=$dato2['horafin'];
			}elseif($dia==$dia1){
				?>
				<?php echo umill(ucwords(strtolower(substr($dia,2))));?>&nbsp;<?php echo umill(substr($horainicio,0,5));?>-<?php echo umill(substr($horafin,0,5));?><br>
				<?php
				$dia1=$dato2['dia'];
				$horainicio=$dato2['horainicio'];
				$horafin=$dato2['horafin'];
...
<tr>
<td><a href="#">Ubicaci&oacute;n</a></td>
<td><a href="#">Fotos</a></td>
<td><a href="#">Visita Virtual</a></td>
<td><a href="#">La Carta</a></td>
<td><a href="#">Eventos</a></td>
</tr>
<tr>
<td colspan="5" align="center"><a href="#" onClick="javascript: window.open('mantReservaUser.php?id_empresa=<?php echo $dato['idempresa'];?>&id_sucursal=<?php echo $dato['idsucursal'];?>','_self');">Reservas</a></td>
</tr>
</table>
<?php
}
...
<table style="display:none"><tr><td width="100%" align="right"><?php if($nro_registros_total==0){echo "No hay registros";}else{echo "Registros del $inicio al $fin (".$mostrar.") de ".$nro_registros_total;}?></td></tr></table>
</td></tr>
