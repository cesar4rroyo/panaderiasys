...
$rst = $objGrilla->consultarultimocierre(0);
...
eval("\$rst2 = \$objGrilla->consultar".$clase.$funcion."Mozo('".$datosGeneral->idmovimiento."','0','".$_GET["idsucursal"]."');");
//$pdf->LlenarTabla_Datos($rst2,$CAMPOS);
while($data=$rst2->fetchObject()){
    $pdf->SetFont("courier",'B',12);
    $pdf->Cell(0,5,"MOZO: ".$data->responsable."  FECHA: ".date("dd/mm/yyyy"),0,1,'C',false);
    $pdf->Ln();
    eval("\$rst3=\$objGrilla->consultarGeneralProductoxMozo(".$data->idresponsable.",".$data->idsucursalresponsable.",'".$datosGeneral->idmovimiento."',0,'".$_GET["idsucursal"]."');");
