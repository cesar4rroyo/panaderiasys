...
header("Content-Disposition: attachment; filename=\"Reporte Diario Almacen.xls\"");
$registro="<table border='1'><thead><tr><th colspan='5'>";
$title='Reporte Diario Almacen del '.$_GET["fechainicio"]." al ".$_GET["fechafin"];
$registro.=$title."</th></tr>";
$registro.="<tr><th colspan='5'>";
$registro.="</th></tr><tr>";
$registro.="<th align='center'>FECHA</th>";
$registro.="<th align='center'>TIPO DOC.</th>";
$registro.="<th align='center'>PERSONA</th>";
$registro.="<th align='center'>MOTIVO</th>";
$registro.="<th align='center'>TOTAL</th>";
$registro.="</tr></thead>";
$sql="select T.idtipodocumento,cast(T.fecha as date) as fecha,sum(T.total) as total,T.motivo,pm.apellidos,pm.nombres
...
$sql.=" group by cast(T.fecha as date),T.idtipodocumento,T.motivo,pm.apellidos,pm.nombres order by cast(T.fecha as date) asc";
$rst = $objProducto->obtenerDataSQL($sql);
while($dat=$rst->fetchObject()){
	$registro.="<tr>";
	$registro.="<td>".date("d/m/Y",strtotime($dat->fecha))."</td>";
	$registro.="<td>".($dat->idtipodocumento==7?'INGRESO':'SALIDA')."</td>";
	$registro.="<td>".$dat->apellidos." ".$dat->nombres."</td>";
	$registro.="<td>".$dat->motivo."</td>";
