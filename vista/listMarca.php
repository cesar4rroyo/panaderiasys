...
function buscar(){
	if($("#cargamant").length>0){
            ...
            setRun('vista/listGrilla2','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Marca&id_clase=<?php echo $id_clase;?>&imprimir=SI&filtro=' + vValor, 'grilla', 'grilla', 'img03');	
        }
}
function ordenar(id){
...
function actualizar(id, idsucursal){
	setRun('vista/mantMarca','&accion=ACTUALIZAR&clase=Marca&id_clase=<?php echo $id_clase;?>&Id=' + id +'&id_cliente='+idsucursal,'cargamant', 'cargamant', 'imgloading03');
}
...
<body>
<?php
$objFiltro = new clsMarca($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstMarca = $objFiltro->obtenerTabla();
	...
	foreach($datoOperaciones as $operacion){
		...
		<div class="col s12 m12 l12 center">
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantMarca', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&id_cliente=<?php echo $id_cliente;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
