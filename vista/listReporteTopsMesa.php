...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ", 0,<?php echo $id_cliente;?>, '" + $("#txtBuscar_Descripcion").val() + "',"+ $("#cboIdSalon").val() + ",'" + $("#txtFechaInicio").val() + "','" + $("#txtFechaFin").val() +"'";
	setRun('vista/listGrillaSinOperacionTops','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Mesa&id_clase=<?php echo $id_clase;?>&funcion=Reporte&filtro=' + vValor + '&imprimir=SI&tiporeporte=Tops&titulo=Mesa&datografico=codigo&fechainicio=' + document.getElementById("txtFechaInicio").value + '&fechafin=' + document.getElementById("txtFechaFin").value, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
