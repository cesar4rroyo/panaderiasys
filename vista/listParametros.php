...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ", 0, <?php echo $id_empresa;?>,'" + document.getElementById("txtBuscar").value + "'";
	setRun('vista/listGrilla3','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=Parametro&id_clase=<?php echo $id_clase;?>&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id, idempresa, idsucursal){
	setRun('vista/mantParametros','&accion=ACTUALIZAR&clase=Parametro&id_clase=<?php echo $id_clase;?>&Id=' + id +'&id_cliente='+idsucursal+'&id_empresa='+idempresa,'cargamant', 'cargamant', 'imgloading03');
}

function eliminar(id, idempresa, idsucursal){
	if(!confirm('Está seguro que desea eliminar el registro?')) return false;
		...
		g_ajaxGrabar.setParameter("txtIdEmpresa", idempresa);
...
<body>
<?php
$objFiltro = new clsParametro($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['R_NombreUsuario'],$_SESSION['R_Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstParametro = $objFiltro->obtenerTabla();
if(is_string($rstParametro)){
	echo "<td colspan=100>Error al Obtener datos de Perfil</td></tr><tr><td colspan=100>".$rstParametro."</td>";
}else{
...
if(is_string($rstOperaciones)){
    ...
    <div class="col s12 m12 l12 center">
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantParametros', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&id_cliente=<?php echo $id_cliente;?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
