...
function buscar(){
	...
	vValor = "'"+vOrder + "'," + vBy + ", 0, <?php echo $id_cliente;?>,'" + document.getElementById("txtBuscar").value + "',<?php echo $_GET['IdProducto'];?>,<?php echo $_GET['IdSucursalProducto'];?>";
	setRun('vista/listGrilla2','&nro_reg=<?php echo $nro_reg;?>&nro_hoja='+document.getElementById("nro_hoj").value+'&clase=ListaUnidad&id_clase=<?php echo $id_clase;?>&filtro=' + vValor, 'grilla', 'grilla', 'img03');
}
function ordenar(id){
...
function actualizar(id, idsucursal){
	setRun('vista/mantListaUnidad','&accion=ACTUALIZAR&clase=ListaUnidad&id_clase=<?php echo $id_clase;?>&Id=' + id +'&id_cliente='+ idsucursal +'&IdProducto=<?php echo $_GET['IdProducto'];?>&IdSucursalProducto=<?php echo $_GET['IdSucursalProducto'];?>','cargamant', 'cargamant', 'imgloading03');
}
...
<body>
<?php //echo "AKI ESTA PRODUCTO". $_GET["IdProducto"]; ?>
<?php
$objFiltro = new clsListaUnidad($id_clase, $_SESSION['R_IdSucursal'],$_SESSION['NombreUsuario'],$_SESSION['Clave']);
?>
<!--BOTONERA INICIO-->
...
$rstListaUnidad = $objFiltro->obtenerTabla();
if(is_string($rstListaUnidad)){
	echo "<td colspan=100>Error al Obtener datos de Perfil</td></tr><tr><td colspan=100>".$rstListaUnidad."</td>";
}else{
	...
	foreach($datoOperaciones as $operacion){
		...
		?>
            ...
            <button class="tooltipped btn-large light-green accent-1 truncate light-green-text text-darken-4" 
                    ...
                    onClick="javascript:setRun('vista/mantListaUnidad', 'accion=NUEVO&id_clase=<?php echo $id_clase;?>&id_cliente=<?php echo $_GET["id_cliente"];?>&IdProducto=<?php echo $_GET['IdProducto'];?>&IdSucursalProducto=<?php echo $_GET['IdSucursalProducto'];?>', 'cargamant','cargamant', 'img04');"><i class="material-icons right">note_add</i><?php echo umill($operacion['descripcion']);?></button>
        </div>
		<?php
		}
	}
