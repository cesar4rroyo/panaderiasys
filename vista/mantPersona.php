...
function cambiaNroDoc(tipopersona){
	if(tipopersona=='VARIOS'){
		document.getElementById("divNroDoc").innerHTML='DNI';
		...
		document.getElementById("divNombres").innerHTML='Nombres';
		document.getElementById("trApellido").style.display="";
		document.getElementById("trSexo").style.display="";
		document.getElementById("trFechaNac").style.display="";
	}else{
		document.getElementById("divNroDoc").innerHTML='RUC';
		document.getElementById("txtNroDoc").size=11;
		document.getElementById("txtNroDoc").maxLength=11;
		if(tipopersona=='JURIDICA'){
			document.getElementById("divNombres").innerHTML='Razon Social';
			document.getElementById("trApellido").style.display="none";
			document.getElementById("trSexo").style.display="none";
			document.getElementById("trFechaNac").style.display="none";
			document.getElementById("txtApellidos").value="";
			document.getElementById("optM").checked="checked";
		}else{
			document.getElementById("divNombres").innerHTML='Nombres';
...
<?php }?>
<?php if($_GET["accion"]=="ACTUALIZAR"){?>
CargarCabeceraRuta([["ACTUALIZAR",'vista/mantPersona','<?php echo $_SERVER["QUERY_STRING"];?>']],false);
<?php }else{?>
CargarCabeceraRuta([["NUEVO",'vista/mantPersona','<?php echo $_SERVER["QUERY_STRING"];?>']],false);
<?php }?>
$("#tablaActual").hide();
$("#opciones").hide();
</script>
    ...
    <div class="container">
        <form id="frmMantPersona" action="" method="POST">
            ...
            echo htmlentities(umill($datoPersonaMaestro["nrodoc"]), ENT_QUOTES, "UTF-8");
                    ...
                    <button type="button" id="btnBuscarRuc" onclick="consultaRUC();" class="btn btn-floating left"><i class="material-icons">find_replace</i></button>
                        <!--button type="button" onClick="buscaPersonaMaestro(txtNroDoc.value,cboTipoPersona.value,'SI')" title="Busca Persona Maestro" <?php if($_GET["accion"]=="ACTUALIZAR") echo 'disabled';?>><img src="img/b_search.png" align="absbottom"></button><sup title="Obligatorio">(&lowast;)</sup> <label id="LabelVerificaNroDoc" style="color: #003399"></label--></td></tr>
            <?php
...
echo htmlentities(umill($datoPersonaMaestro[strtolower($value["descripcion"])]), ENT_QUOTES, "UTF-8");
	...
	<td><?php echo $value["comentario"];?></td>
    	<td>
            <input type="date" id="txt<?php echo $value["descripcion"];?>" name = "txt<?php echo $value["descripcion"];?>" value = "<?php if($_GET["accion"]=="ACTUALIZAR")
echo htmlentities(umill($datoPersonaMaestro[strtolower($value["descripcion"])]), ENT_QUOTES, "UTF-8");else echo date("Y-m-d");?>" size="10" maxlength="10">
...
echo htmlentities(umill($datoPersonaMaestro[strtolower($value["descripcion"])]), ENT_QUOTES, "UTF-8");else echo date("d/m/Y");*/?>" size="10" maxlength="10"><button id="btnCalendar" type="button" class="boton"><img src="img/date.png" width="16" height="16"> </button--></td></tr>
	<?php }?>
<?php }
//CAMPOS DEL ALUMNO
reset($dataCampos);
	...
	<tr><td><?php echo $value["comentario"];?></td>
        <td>
            <p>
                ...
                <label for="chkCompartido">Compartido?</label>
            </p>
	</td>
    	</tr>
	<?php }?>
