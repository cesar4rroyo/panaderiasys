...
<?php
	...
	<td class="center">
            <div class="col s12">
                <?php
                if($origen=="Venta"){
                	if($dato["idtipodocumento"]=="19"){
                ?>
                <button type="button" style="margin-left: 10px;" class="btn-floating btn-large tooltipped blue accent-1" data-position="bottom" data-delay="50" data-tooltip="CAMBIAR A BOLETA" onClick="javascript:if(confirm('Desea cambiarlo a boleta?')){cambiarComprobante(<?php echo $dato["idmovimiento"];?>);}"><i class="material-icons green-text text-darken-4">input</i></button>
                <?php
                	}
                ?>
	...
	if($dato['estado']!='I'){
    	...
    	foreach($datoOperaciones as $operacion){
    		if($operacion["tipo"]=="C"){
                        if($operacion["idoperacion"]==2 || $operacion["accion"]=="actualizar"){
                                ...
                                if($id_clase==34){
                                    $operacion["imagen"] = "assignment_ind";
                                }else{
...
<div class="row">
    <form id="frmDatosReporte" method="post" target="_blank" action="vista/reportes/Reporte<?php echo $tiporeporte;?>.php">
        ...
        <input id="txtNroRegistrosTotalREPORTE" name="txtNroRegistrosTotalREPORTE" type="hidden" value="<?php echo $nro_reg;?>">
