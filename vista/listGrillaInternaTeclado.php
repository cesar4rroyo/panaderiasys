...
?>
<th><a href="#" onClick="javascript:ordenar<?php echo $nombre?>('<?php echo umill($value['descripcion']);?>');"><?php echo umill($value['comentario'])?></a></th>
<?php 
}
//<<Fin
...
while($dato = $rst->fetch()){
	...
	$c+=1;
?>
<tr id='<?php echo $dato[1];?>' class="<?php if($c%2==0) echo 'par'; else echo 'impar';?>">
<?php
	foreach($dataCampos as $value){
?>
...
<?php
	...
	foreach($datoOperaciones as $operacion){
		if($operacion["tipo"]=="C"){
		?>
<td><a href="#" title="<?php echo umill($operacion["comentario"]);?>" onClick="javascript:<?php echo umill($operacion["accion"]);?>(<?php echo $dato[1];?>);"><?php echo umill($operacion["descripcion"]);?></a></td>
		<?php
		}
	}
?>
...
<tr>
<?php
$ini = "<td><a href=\"#\" onClick=\"buscarGrillaInterna(";
$medio=")\">";
if($nro_hojas>11){
	for($i=1;$i<=3;$i++){
...
<table><tr><td width="100%" align="right"><?php if($nro_registros_total==0){echo "No se hay registros";}else{echo "Registros del $inicio al $fin (".$mostrar.") de ".$nro_registros_total;}?></td><td><a href="#" onClick="javascript: ocultarResultadoListGrillaInterna()" title="Cerrar">X</a></td></tr></table>
</td></tr></table>
</td></tr></table>
</body>
</HTML>
