...
echo htmlentities(umill($datoPersonaMaestro[strtolower($value["descripcion"])]), ENT_QUOTES, "UTF-8");
	?>" style="text-transform:uppercase" <?php if($_GET["accion"]=="ACTUALIZAR") echo 'disabled';?> <?php if($value["validar"]=='S' and !($value["msgvalidar"]=='' or empty($value["msgvalidar"]))){echo "title='".$value["msgvalidar"]."'";}?> <?php if(!($value["longitud"]==0 or empty($value["longitud"]))){echo "maxlength=".$value["longitud"];}?> <?php if(!($value["longitud"]==0 or empty($value["longitud"]))){if($value["longitud"]<=100){echo "size=".$value["longitud"];}else{echo "size=100";}}?>></td></tr>
	<?php }?>
...
echo htmlentities(umill($datoPersonaMaestro[strtolower($value["descripcion"])]), ENT_QUOTES, "UTF-8");
	...
	<tr id="trSexo" style="display:none"><td><?php echo $value["comentario"];?></td>
    	<td>
    	  <label>
    	    <input name="opt<?php echo $value["descripcion"];?>" type="radio" id="optM" value="M" <?php if($_GET["accion"]=="ACTUALIZAR"){if($datoPersonaMaestro[strtolower($value["descripcion"])]=="M"){ echo "checked=checked";}}else{ echo "checked=checked";}?> <?php if($_GET["accion"]=="ACTUALIZAR") echo 'disabled';?>>
    	    M</label>
    	  <label>
    	    <input name="opt<?php echo $value["descripcion"];?>" type="radio" id="optF" value="F" <?php if($_GET["accion"]=="ACTUALIZAR"){if($datoPersonaMaestro[strtolower($value["descripcion"])]=="F"){ echo "checked=checked";}}?> <?php if($_GET["accion"]=="ACTUALIZAR") echo 'disabled';?>>
    	    F</label></td></tr>
	<?php }?>
<?php }
//CAMPOS DEL ALUMNO
